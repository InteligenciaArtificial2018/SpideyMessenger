...
import kotlinx.android.synthetic.main.activity_registro_chat.*

class RegistroChatActivity : AppCompatActivity() {
  ...
  private fun listaMensajes() {
    ...
    ref.addChildEventListener(object: ChildEventListener {
      override fun onChildAdded(p0: DataSnapshot, p1: String?) {
        val mensajeChat = p0.getValue(MensajesChat::class.java)

        if (mensajeChat != null) {
          Log.d(TAG, mensajeChat.texto)

          if (mensajeChat.deId == FirebaseAuth.getInstance().uid) {
            val usuarioActual = UltimosMensajesActivity.usuarioActual?: return
  ...
  private fun realizarEnvioMensajes() {
    ...
    val mensajeChat = MensajesChat(ref.key!!, texto, deId, paraId, System.currentTimeMillis() / 1000)
